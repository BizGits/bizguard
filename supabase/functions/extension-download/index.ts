...
function showWarning(term, brandName, element) {
  ...
  title.textContent = 'Brand Mismatch!';
  const msg = document.createElement('p');
  msg.textContent = 'Term from ';
  const bn = document.createElement('strong');
  bn.textContent = brandName;
  msg.appendChild(bn);
  msg.appendChild(document.createTextNode(' detected while on '));
  const cb = document.createElement('strong');
  cb.textContent = currentBrand.name;
  msg.appendChild(cb);
  const termP = document.createElement('p');
  termP.className = 'bizguard-term';
  termP.textContent = '"' + term + '"';
  textDiv.appendChild(title);
  textDiv.appendChild(msg);
  textDiv.appendChild(termP);
  const btn = document.createElement('button');
  btn.className = 'bizguard-dismiss';
  btn.textContent = 'Dismiss';
  btn.addEventListener('click', (e) => { e.stopPropagation(); warning.remove(); });
